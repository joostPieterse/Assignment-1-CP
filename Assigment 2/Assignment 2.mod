...
{DemandStep} DemSteps =
{<d,s> | d in Demands, s in Steps 
        : d.productId == s.productId};  

{DemandAlternative} DemandAlternatives =
{<d,a> | d in Demands, a in Alternatives 
        : d.productId==item(Steps, <a.stepId>).productId};  
...
subject to{
	...
	forall(st in StorageTanks){
		...
		forall(s in Steps, sp in StorageProductions, d in Demands:d.productId == s.productId&&s.stepId == sp.prodStepId && st.storageTankId == sp.storageTankId){
			...
			alwaysEqual(state[st], storageResourcesAlternatives[<d,s>][st], s.productId, 1, 1);
		}	
	}
}
